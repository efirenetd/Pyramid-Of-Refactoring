...
import static pl.refactoring.interpreter.completed.spec.Specs.placeIn;
...
public class RealEstateFinder {
    ...
    public List<RealEstate> byMaterialBelowArea(EstateMaterial material, float maxBuildingArea){
        return bySpec(new AndSpecBuilder().withSpecs(ofMaterial(material))
                .withSpecs(belowArea(maxBuildingArea)).build());
    }

    @Deprecated
    public List<RealEstate> byPlacement(EstatePlacement placement){
        return bySpec(placeIn(placement));
    }

    @Deprecated
    public List<RealEstate> byAvoidingPlacement(EstatePlacement placement){
        return bySpec(not(placeIn(placement)));
    }
    ...
    public List<RealEstate> byTypePlacementMaterial(EstateType type, EstatePlacement placement, EstateMaterial material){
        return bySpec(new AndSpecBuilder()
                .withSpecs(ofType(type))
                .withSpecs(placeIn(placement))
                .withSpecs(ofMaterial(material))
                .build());
    }
